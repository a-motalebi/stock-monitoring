...
def Market_with_askbid():
    ...
    for item in inst_price:
        item = item.split(",")
        market_me[item[0]] = dict(
            id=item[0],
            isin=item[1],
            symbol=item[2],
            name=item[3],
            open_price=float(item[5]),
            close_price=float(item[6]),
            last_trade=float(item[7]),
            count=item[8],
            volume=float(item[9]),
            value=float(item[10]),
            low_price=float(item[11]),
            high_price=float(item[12]),
            yesterday_price=int(item[13]),
            table_id=item[17],
            group_id=item[18],
            max_allowed_price=float(item[19]),
            min_allowed_price=float(item[20]),
            last_return=(float(item[7]) - float(item[13])) / float(item[13]),
            close_ret=(float(item[6]) - float(item[13])) / float(item[13]),
            number_of_shares=float(item[21]),
            market_cap=int(item[21]) * int(item[6]),
        )
